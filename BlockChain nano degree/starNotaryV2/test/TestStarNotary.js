...
it("can add the star name and star symbol properly", async () => {
  ...
  let tokenId = 100;
  let instance = await StarNotary.deployed();
  const res = await instance.createStar("Deneb", tokenId, {
    from: accounts[0],
  });
  ...
  assert.equal(name, "Perla");
  assert.equal(symbol, "Olx");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  await instance.createStar("Altair", tokenId1, { from: user1 });
  await instance.createStar("Deneb", tokenId2, { from: user2 });
...
it("lets a user transfer a star", async () => {
  ...
  // 1. create a Star with different tokenId
  let tokenId = 103;
  await instance.createStar("Deneb", tokenId, { from: user1 });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  // 1. create a Star with different tokenId
  await instance.createStar("Altair", tokenId, { from: user1 });

  // 2. Call your method lookUptokenIdToStarInfo
  const name = await instance.lookUptokenIdToStarInfo(tokenId);

  // 3. Verify if you Star name is the same
  assert.equal(name, "Altair");
});

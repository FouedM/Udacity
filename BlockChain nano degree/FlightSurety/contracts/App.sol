...
contract FlightSuretyApp {
    ...
    uint256 public constant MIN_INSURANCE_FEE = 0 ether;
        ...
        if(airlinesRegisteredCount < MULTIPARTY_CONSENSUS_COUNT) {
            ...
            if(success) {
                airlinesRegisteredCount ++;
                  ...
                  mapping(address=> bool) storage supportingAirlines = airlinePolls[airline];
    ...
    *
    */
    function AirlineFunding () public payable requireIsOperational requireIsAirlineRegistered(msg.sender)
    {
        // Transfer Fund to Data Contract
        _make_payable(address(_flightSuretyData)).transfer(msg.value);
    ...
    */
    function registerFlight(address airline, string memory flight, uint timestamp)
                                                            public
                                                            payable
                                                            requireIsOperational
       ...
       require(msg.value > MIN_INSURANCE_FEE, "Insurance fee must be more than 0 ether");
...
// region ORACLE MANAGEMENT
    ...
    function getExistingAirlines () public view requireIsOperational returns(address[] memory) {
        return _flightSuretyData.getAirlines();
    }

    function getAirlineFunds (address airline) public view requireIsOperational returns(uint funds) {
        return _flightSuretyData.getAirlineFunds(airline);
    }

    function getBalance () public view requireIsOperational returns(uint funds) {
        return _flightSuretyData.getPassengerFunds(msg.sender);
    }

    function withdrawFunds (uint amount) public requireIsOperational returns(uint funds) {
...
contract FlightSuretyData {
    ...
    function buy(address airline, string calldata flight, uint256 timestamp,address passenger, uint256 amount) external;
    function creditInsurees(address airline, string calldata flight, uint256 timestamp,uint factor_numerator,uint factor_denominator) external;

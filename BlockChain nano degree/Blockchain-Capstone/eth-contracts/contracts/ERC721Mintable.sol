...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, 'Unauthorzied Caller');
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), 'newOwner must be a real address');
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(!_paused, 'The contract shouldnt be paused');
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), 'It must be transfered to another address');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == ownerOf(tokenId), 'The sender must be the owner');
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        require(_exists(tokenId), 'Unexist token');
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, 'The receiver shouldnt be the sender');
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), 'The sender should be approved or owner');
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(address(0) != to, 'The receiver must be a vallid address');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), 'Only the owner could transfer the token');

        // TODO: require token is being transfered to valid address
        require(to != address(0), 'The receiver must be a vallid address');
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenUri(uint256 tokenId) internal {
        require(_exists(tokenId), 'Unexisted token');
        string memory newToken = strConcat(_baseTokenURI, uint2str(tokenId));
        _tokenURIs[tokenId] = newToken;
    }
...
contract CustomERC721Token is ERC721Metadata("AZMOUR", "ZITOUN", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
